...
export const SUPPORTED_BACKGROUND_TYPES: Record<string, { label: string, value: string }> = {
...
export const backgroundTransformations:
  Record<string, ({ context, transformationGroup }: { context: BackgroundContext, transformationGroup: string | undefined }) => TransformationField> = {
    backgroundType: ({ context, transformationGroup }) => {
      ...
      if (context === "root_image") {
        ...
        availableTypes = availableTypes.filter(type => 
          type.value === "color" || type.value === "gradient"
        );
      } else if (context === "pad_extract") {
        // Pad extract: color, gradient, and generative fill (no blur)
        availableTypes = availableTypes.filter(type => 
          type.value === "color" || type.value === "gradient" || type.value === "generative_fill"
        );
      }
    ...
    background: ({ context, transformationGroup }) => {
      ...
      switch (context) {
        case "root_image":
          ...
          transformation.isVisible = ({ backgroundType, backgroundDominantAuto }) => 
    ...
    backgroundDominantAuto: ({ transformationGroup }) => {
      const transformation: TransformationField = {
        ...
        helpText: "Automatically pick the most dominant color from the border pixels of the image. Useful when using pad_resize or pad_extract crop modes.",
        isVisible: ({ backgroundType }) => backgroundType === "color",
      }
    ...
    backgroundGradientAutoDominant: ({ transformationGroup }) => {
      const transformation: TransformationField = {
        ...
        isTransformation: false,
        transformationGroup: transformationGroup,
        helpText: "Automatically pick the most dominant colors from the image to form a gradient background.",
    ...
    backgroundGradientMode: ({ transformationGroup }) => {
      const transformation: TransformationField = {
        ...
        isVisible: ({ backgroundType, backgroundGradientAutoDominant }) => 
          backgroundType === "gradient" && backgroundGradientAutoDominant === true,
      }
    ...
    backgroundGradientPaletteSize: ({ transformationGroup }) => {
      const transformation: TransformationField = {
        ...
        helpText: "Number of colors to pick from the image. 2 creates a linear gradient, 4 creates corner interpolation.",
        isVisible: ({ backgroundType, backgroundGradientAutoDominant }) => 
          backgroundType === "gradient" && backgroundGradientAutoDominant === true,
      }
...
export const background = {
  ...
  getPropsFor: (context: BackgroundContext): {
    schema: Record<string, z.ZodTypeAny>,
    transformations: ({ transformationGroup }: { transformationGroup: string | undefined }) => TransformationField[],
  } => {
    ...
    switch (context) {
      case "root_image":
        ...
        schemaToBeReturned = {
          ...
          backgroundGradientAutoDominant: background.schemas.backgroundGradientAutoDominant,
          backgroundGradientMode: background.schemas.backgroundGradientMode,
          backgroundGradientPaletteSize: background.schemas.backgroundGradientPaletteSize,
    ...
    const transformationsToBeReturned = (transformationGroup: string | undefined) => Object.entries(backgroundTransformations)
    ...
    return {
      ...
      transformations: ({ transformationGroup }: { transformationGroup: string }) =>
