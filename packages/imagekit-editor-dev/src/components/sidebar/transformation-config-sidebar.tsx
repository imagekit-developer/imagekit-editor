...
import { DEFAULT_FOCUS_OBJECTS, transformationSchema, RESIZE_CROP_MODES, RESIZE_CROP_HELP_TEXT } from "../../schema"
...
export const TransformationConfigSidebar: React.FC = () => {
  ...
  const onApply = (data: Record<string, unknown>) => {
    ...
    if (selectedTransformation.key === "resize_and_crop-resize_and_crop" && data.mode) {
    ...
    const isAutoGeneratedName = (name: string) => {
      ...
      return RESIZE_CROP_MODES.some(mode => 
        name === `Resize and Crop (${mode.label})`
      )
    }
    ...
    if (transformationToEdit && transformationToEdit.position === "inplace") {
      ...
      if (selectedTransformation.key === "resize_and_crop-resize_and_crop" && editedTransformation?.name) {
                  ...
                  {selectedTransformation.key === "resize_and_crop-resize_and_crop" &&
                    ...
                    values.height && (
                      <Text as="span" color="orange.500" display="block" mt={1}>
                        Note: Aspect ratio is ignored when both width and height are specified.
                      </Text>
                    )}
                </FormHelperText>
              )}
