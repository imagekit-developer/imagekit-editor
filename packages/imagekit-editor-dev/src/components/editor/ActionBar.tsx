...
export const ActionBar: FC<ActionBarProps> = ({
  ...
  const imageDimensions = useMemo(() => {
    if (isCanvasOnly) return { width: canvas.width, height: canvas.height }
    const idx = imageList.findIndex((img) => img === currentImage)
    if (idx === -1) return null
    return originalImageList[idx]?.imageDimensions ?? null
  }, [isCanvasOnly, canvas, currentImage, imageList, originalImageList])
        ...
        {viewMode === "list" && imageDimensions && (
          <>
            ...
            <Text
              ...
              paddingX="4"
            >
              {isCanvasOnly ? "Canvas" : "Dimensions"}:{" "}
