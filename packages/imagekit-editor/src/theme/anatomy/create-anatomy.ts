 ...
 */
export function anatomy<T extends string = string>(name: string, map = {} as any): Anatomy<T> {
  let called = false;
   ...
   */
  function selectors() {
    const value = Object.fromEntries(Object.entries(map).map(([key, part]) => [key, (part as any).selector]));
   ...
   */
  function classnames() {
    const value = Object.fromEntries(Object.entries(map).map(([key, part]) => [key, (part as any).className]));
    return value as Record<T, string>;
  }
